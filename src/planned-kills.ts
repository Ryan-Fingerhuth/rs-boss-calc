...
export function parsePlannedKills(input: string): number[] | null {
  ...
  if (parts.length > MAX_PLANS || parts.some(part => !/^\d+$/.test(part))) return null
  const values = parts.map(Number)
  if (values.some(value => !Number.isSafeInteger(value) || value > MAX_KILLS)) return null
  return [...new Set(values)].sort((a, b) => a - b)
}

export function addPlannedKills(input: string, count: number): string {
  const plans = parsePlannedKills(input)
  if (!plans || !Number.isInteger(count) || count < 0 || count > MAX_KILLS) return input
