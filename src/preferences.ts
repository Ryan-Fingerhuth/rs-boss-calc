...
export function restorePreferences(raw: string | null): Preferences {
  try {
    ...
    const scenario =
      boss.scenarios.find((s) => s.name === saved.scenario) ??
      boss.scenarios[0];
    // An unavailable target remains selected if it exists in another scenario.
    const knownDrop = dropOptions(boss, scenario).some(
      (d) => d.name === saved.drop,
    );
    const text = (key: keyof Preferences) =>
      typeof saved[key] === "string" && saved[key].length <= 100
        ? (saved[key] as string)
    ...
    return {
      ...
      customRate: text("customRate"),
      customThreshold: text("customThreshold"),
      advanced:
        typeof saved.advanced === "boolean" ? saved.advanced : !!boss.custom,
    };
  } catch {
...
export function readPreferences(): Preferences {
  try {
    return restorePreferences(window.localStorage.getItem(STORAGE_KEY));
  } catch {
    return { ...defaultPreferences };
  }
}
...
export function savePreferences(preferences: Preferences): void {
  ...
  try {
    ...
    /* Keep the current in-memory selections. */
  }
}
